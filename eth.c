  ...
  mac_addr_t dst, uint16_t type, unsigned char * payload, int payload_len )
{
  ...
  // Padding Ethernet
    ...
    /*if(eth_frame_len < 60) {
      ...
      //int diff = 60 - eth_frame_len;
      //unsigned char padding[60];

      for(int i = 0; i<60-14; i++) {
        //padding[i] = 0;
        eth_frame.payload[i] = 0xFF;
      }

      //unsigned char cat[ETH_MTU];
      //strcat(cat, payload);
      //strcat(cat, padding);
      ...
      eth_frame_len = 60;
      //memcpy(eth_frame.payload, cat, ETH_MTU);
  ...
  int buf_len, long int timeout )
{
  ...
  do {
    ...
    multicast = ((eth_frame_ptr->dest_addr[0] & 0x01) == 0x01);

  } while ( ! ((is_my_mac || multicast) && is_target_type) );
